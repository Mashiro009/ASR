...
# other reference https://blog.csdn.net/simondavier/article/details/82385700
...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    q.put(in_data)
    audioframe = np.frombuffer(in_data, np.dtype('<i2'))
    endpointing.classifyFrame(audioframe)
    ad_rdy_ev.set()
    if endpointing.isSlienceTooLong():
        print(endpointing.continueSlienceTime)
        print("complete")
        plt.close()
...
def read_audio_thead(q, stream, frames, ad_rdy_ev):
    ...
    while stream.is_active():
        ...
        if not q.empty():
            ...
            # endpointing.classifyFrame(rt_data)
